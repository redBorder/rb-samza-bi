...
public class EventGoldBeamFactory implements BeamFactory
{
    @Override
    public Beam<Object> makeBeam(SystemStream stream, Config config)
    {
        final int maxRows = Integer.valueOf(config.get("redborder.beam.event.gold.maxrows", "200000"));
        final int partitions = Integer.valueOf(config.get("redborder.beam.event.gold.partitions", "2"));
        final int replicas = Integer.valueOf(config.get("redborder.beam.event.gold.replicas", "1"));
        final String intermediatePersist = config.get("redborder.beam.event.gold.intermediatePersist", "PT20m");
