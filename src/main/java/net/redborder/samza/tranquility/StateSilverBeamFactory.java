...
public class StateSilverBeamFactory implements BeamFactory
{
    @Override
    public Beam<Object> makeBeam(SystemStream stream, Config config)
    {
        final int maxRows = Integer.valueOf(config.get("redborder.beam.state.silver.maxrows", "200000"));
        final int partitions = Integer.valueOf(config.get("redborder.beam.state.silver.partitions", "2"));
        final int replicas = Integer.valueOf(config.get("redborder.beam.state.silver.replicas", "1"));
        final String intermediatePersist = config.get("redborder.beam.state.silver.intermediatePersist", "PT20m");
