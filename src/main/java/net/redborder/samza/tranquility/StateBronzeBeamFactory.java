...
public class StateBronzeBeamFactory implements BeamFactory
{
    @Override
    public Beam<Object> makeBeam(SystemStream stream, Config config)
    {
        final int maxRows = Integer.valueOf(config.get("redborder.beam.state.bronze.maxrows", "200000"));
        final int partitions = Integer.valueOf(config.get("redborder.beam.state.bronze.partitions", "2"));
        final int replicas = Integer.valueOf(config.get("redborder.beam.state.bronze.replicas", "1"));
        final String intermediatePersist = config.get("redborder.beam.state.bronze.intermediatePersist", "PT20m");
