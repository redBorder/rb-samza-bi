...
public class EventSilverBeamFactory implements BeamFactory
{
    @Override
    public Beam<Object> makeBeam(SystemStream stream, Config config)
    {
        final int maxRows = Integer.valueOf(config.get("redborder.beam.event.silver.maxrows", "200000"));
        final int partitions = Integer.valueOf(config.get("redborder.beam.event.silver.partitions", "2"));
        final int replicas = Integer.valueOf(config.get("redborder.beam.event.silver.replicas", "1"));
        final String intermediatePersist = config.get("redborder.beam.event.silver.intermediatePersist", "PT20m");
        ...
        final List<String> dimensions = ImmutableList.of(
            ...
            SENSOR_NAME, SENSOR_ID, DEPLOYMENT_ID, PRIORITY, MSG, SIG_ID,
