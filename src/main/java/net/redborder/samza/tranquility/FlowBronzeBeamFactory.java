...
public class FlowBronzeBeamFactory implements BeamFactory
{
    @Override
    public Beam<Object> makeBeam(SystemStream stream, Config config)
    {
        ...
        final int partitions = Integer.valueOf(config.get("redborder.beam.flow.bronze.partitions", "2"));
        final int replicas = Integer.valueOf(config.get("redborder.beam.flow.bronze.replicas", "1"));
