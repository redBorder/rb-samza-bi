...
public class AppMobileCallBeamFactory implements BeamFactory {
    ...
    {
        int maxRows = Integer.valueOf(config.get("redborder.beam.appmobile_call.maxrows", "200000")).intValue();
        String intermediatePersist = config.get("redborder.beam.appmobile_call.intermediatePersist", "PT20m");
        String zkConnect = config.get("systems.kafka.consumer.zookeeper.connect");
        long indexGranularity = Long.valueOf(config.get("systems.druid_appmobile_call.beam.indexGranularity", "60000")).longValue();

        String dataSource = stream.getStream();

        List<String> dimensions = ImmutableList.of("timezone", "IMEI", "number", "type");

        List<AggregatorFactory> aggregators = ImmutableList.of(
                new DoubleMinAggregatorFactory("min_duration", "duration"),
                new DoubleMaxAggregatorFactory("max_duration", "duration"),
                new DoubleSumAggregatorFactory("sum_duration", "duration"),
