        ...
        try {
            if (conn != null) {
                ...
                rs = st.executeQuery("SELECT DISTINCT ON (access_points.mac_address) access_points.ip_address," +
                        ...
                        " service_providers.uuid AS service_provider_uuid, vehicles.name AS vehicle," +
                        ...
                        " (SELECT * FROM sensors WHERE domain_type=3) AS markets ON markets.lft <= sensors.lft AND " +
                        " markets.rgt >= sensors.rgt LEFT JOIN (SELECT * FROM sensors WHERE domain_type=2) AS" +
                        " organizations ON organizations.lft <= sensors.lft AND organizations.rgt >= sensors.rgt LEFT " +
                        " JOIN (SELECT * FROM sensors WHERE domain_type=6) AS service_providers ON service_providers.lft " +
                        " <= sensors.lft AND service_providers.rgt >= sensors.rgt LEFT JOIN " +
                        " (SELECT * FROM sensors WHERE domain_type=9) AS vehicles ON vehicles.lft <= " +
                        " sensors.lft AND vehicles.rgt >= sensors.rgt;");
