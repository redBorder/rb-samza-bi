        ...
        try {
            if (conn != null) {
                ...
                rs = st.executeQuery("SELECT DISTINCT ON (access_points.mac_address) access_points.ip_address," +
                        ...
                        "                   markets.uuid AS market_uuid, organizations.name AS organization," +
                        "                   organizations.uuid AS organization_uuid," +
                        "                   service_providers.name AS service_provider," +
                        "                   service_providers.uuid AS service_provider_uuid" +
                        "      FROM access_points LEFT JOIN sensors ON (access_points.sensor_id = sensors.id AND access_points.switch_id is null" +
                        "                                               OR access_points.sensor_id = sensors.id" +
